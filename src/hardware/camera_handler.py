...
# # #     def __init__(self, model_path, conf_threshold=0.6, move_threshold_px=5, stop_threshold_sec=30, parent=None):
...
# # #                 hand_detected = len(results[0].boxes) > 0

# # #                 if hand_detected:
# # #                     best_box = results[0].boxes[0]; xyxy = best_box.xyxy[0].cpu().numpy()
# # #                     current_position = np.array([(xyxy[0] + xyxy[2]) / 2, (xyxy[1] + xyxy[3]) / 2])
...
# # #                             self.last_activity_time = time.time(); self.signal_emitted = False
# # #                     else:
# # #                         self.last_activity_time = time.time(); self.signal_emitted = False
...
# # #                     print(f"{self.stop_threshold}秒間、手の活動がありませんでした。シグナルを発行します。")
...
# # #                 # --- ここからが修正箇所 ---
# # #                 # OpenCVのフレーム(BGR)を、QtのQImage(BGRA)に変換
# # #                 # これにより、4バイトアライメントが保証される
# # #                 bgra_frame = cv2.cvtColor(annotated_frame, cv2.COLOR_BGR2BGRA)
# # #                 h, w, ch = bgra_frame.shape
# # #                 bytes_per_line = ch * w
# # #                 # フォーマットをFormat_ARGB32に変更
# # #                 qt_image = QImage(bgra_frame.data, w, h, bytes_per_line, QImage.Format_ARGB32)
...
# # # src/hardware/camera_handler.py YOLOの分析結果を渡すだけに変更
...
# #     描画処理は一切行わない。
# #     """
# #     # 描画に必要な全ての情報をまとめて送信する新しいシグナル
# #     # 引数: (描画対象のQImage, 検出結果のリスト)
# #     frame_data_ready = Signal(QImage, list)
    
# #     # 定点観測用の、未加工の生フレーム (PIL.Image形式)
# #     raw_frame_for_observation = Signal(PILImage.Image)
    
# #     # 手が停止した瞬間の、未加工の生フレーム (PIL.Image形式)
# #     hand_stopped_signal = Signal(PILImage.Image)

# #     def __init__(self, model_path, conf_threshold=0.6, move_threshold_px=5, stop_threshold_sec=60, parent=None):
# #         super().__init__(parent)
# #         self.camera_index =  0#2でPC本体　1でサードパーティ。スマホカメラ
# #         self.is_running = True
...
# #                 if hand_detected:
# #                     # 最も信頼度が高い検出結果（YOLOの推論結果は通常、信頼度順にソートされている）
...
# # src/hardware/camera_handler.py　使用カメラの指定
...
#     def __init__(self, model_path, device_index=0, conf_threshold=0.6, move_threshold_px=5, stop_threshold_sec=60, parent=None):
#         super().__init__(parent)
