...
# # # #     """
# # # #     SQLiteデータベースの操作（接続、テーブル作成、CRUD）をカプセル化するクラス。
# # # #     時間的コンテキストの取得機能も含む。
# # # #     """
...
# # # #         # `__init__`でテーブルの存在とカラムの存在を両方チェック・修正する
...
# # # #         """データベースとテーブルの初期化、およびマイグレーションを行う"""
...
# # # #         )
# # # #         """)
# # # #         # last_updated_at カラムの存在をチェックし、なければ追加（マイグレーション処理）
...
# # # #         conn.commit()
# # # #         conn.close()
# # # #         print("データベーステーブルの初期化を確認・完了しました。")
...
# # # #         now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
# # # #         if title is None: title = f"チャット - {now}"
# # # #         conn = self._get_connection()
# # # #         cursor = conn.cursor()
# # # #         cursor.execute("INSERT INTO sessions (title, created_at, last_updated_at) VALUES (?, ?, ?)", (title, now, now))
# # # #         session_id = cursor.lastrowid
# # # #         conn.commit()
# # # #         conn.close()
# # # #         print(f"新しいセッションを作成しました (ID: {session_id})")
# # # #         return session_id
...
# # # #         conn = self._get_connection()
# # # #         cursor = conn.cursor()
# # # #         cursor.execute("INSERT INTO messages (session_id, role, content, timestamp) VALUES (?, ?, ?, ?)", (session_id, role, content, now))
# # # #         conn.commit()
# # # #         conn.close()
...
# # # #         conn = self._get_connection()
# # # #         cursor = conn.cursor()
# # # #         cursor.execute("INSERT INTO logs (session_id, log_type, content, timestamp) VALUES (?, ?, ?, ?)", (session_id, log_type, content, now))
# # # #         conn.commit()
# # # #         conn.close()
# # # #         self._update_session_timestamp(session_id)
...
# # # #         conn = self._get_connection()
# # # #         cursor = conn.cursor()
# # # #         cursor.execute("SELECT id, title, created_at, last_updated_at, problem_context, chat_summary FROM sessions WHERE id = ?", (session_id,))
# # # #         row = cursor.fetchone()
# # # #         conn.close()
# # # #         if row: return {"id": row[0], "title": row[1], "created_at": row[2], "last_updated_at": row[3], "problem_context": row[4], "chat_summary": row[5]}
# # # #         return None
...
# # # # src/core/database_manager.py　キーワード抽出、登録
...
# # #         # カラムの存在をチェックし、なければ追加する堅牢なマイグレーション処理
# # #         cursor.execute("PRAGMA table_info(sessions)")
# # #         columns = [column[1] for column in cursor.fetchall()]
...
# # # src/core/database_manager.py　動的検索機能追加
...
# #         # キーワードごとにLIKE句を作成
# #         sql_likes = " OR ".join([f"keywords LIKE ?" for _ in query_keywords])
# #         params = [f"%{kw.strip()}%" for kw in query_keywords] + [exclude_session_id]
        
# #         query = f"""
# #         SELECT id, title, keywords, last_updated_at
# #         FROM sessions
# #         WHERE ({sql_likes}) AND id != ?
# #         """
...
# #             score = 0
# #             # DBのキーワードもカンマで分割してリスト化
# #             session_keywords = [kw.strip() for kw in session_row["keywords"].split(',')]
# #             for q_kw in query_keywords:
# #                 if q_kw.strip() in session_keywords:
# #                     score += 1
# #             if score > 0:
...
# # src/core/database_manager.py　動的検索　改良版
...
#         # まずは全ての過去セッションを取得
#         cursor.execute(
#             "SELECT id, title, keywords, last_updated_at FROM sessions WHERE id != ?",
#             (exclude_session_id,)
#         )
